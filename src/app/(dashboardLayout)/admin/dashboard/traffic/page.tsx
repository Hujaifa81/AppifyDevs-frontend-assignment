...
const AdminTrafficPage = () => {
  ...
  const handleExport = useCallback(() => {
    ...
    exportToCSV(exportData, 'admin_traffic_report');
  }, [traffic, period]);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-10">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b pb-6">
        <div>
          ...
          <p className="text-lg text-muted-foreground font-medium italic">Visitor sources and acquisition patterns.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {loading ? (
          ...
          <DashboardStatus type="error" title="Traffic Data Load Failed" onRetry={refresh} />
            ...
            <div className="lg:col-span-1">
              <Card className="border-none shadow-sm bg-card/50 backdrop-blur-sm border border-border/50 overflow-hidden h-full">
                <CardHeader className="px-6 py-6 border-b border-border/50 bg-muted/20">
                  <CardTitle className="text-xl font-bold">Traffic Breakdown</CardTitle>
                  <CardDescription className="text-sm font-medium">Detailed visitor counts and share per acquisition channel.</CardDescription>
                </CardHeader>
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="bg-muted/30 text-left">
                          ...
                          <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground/70 text-right">Visitors</th>
                          <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground/70 text-right">Share</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border/50">
                        {traffic.sources.map((s) => (
                          <tr key={`traffic-source-${s.source}`} className="group hover:bg-muted/20 transition-colors">
