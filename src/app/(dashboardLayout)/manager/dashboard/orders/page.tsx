...
const ManagerOrders = () => {
  ...
  const handleExport = useCallback(() => {
    ...
    exportToCSV(exportData, 'manager_orders_report');
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b pb-6">
        <div>
          ...
          <p className="text-lg text-muted-foreground font-medium italic">Manage and monitor sales performance.</p>
        </div>
          ...
          <div className="space-y-8">
            ...
            <Card className="border-none shadow-sm bg-card/50 backdrop-blur-sm border border-border/50 overflow-hidden">
              <CardHeader className="px-6 py-6 border-b border-border/50 bg-muted/20">
                <CardTitle className="text-xl font-bold">Detailed Logs</CardTitle>
                <CardDescription className="text-sm font-medium">Historical order counts for the selected period.</CardDescription>
              </CardHeader>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="bg-muted/30 text-left">
                        <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground/70">Period Label</th>
                        <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-muted-foreground/70 text-right">Order Count</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border/50">
                      {orders.data.map((d) => (
                        <tr key={`mgr-order-${d.label}`} className="group hover:bg-muted/20 transition-colors">
                          <td className="px-6 py-4">
                            <span className="text-sm font-bold text-foreground tracking-tight">{d.label}</span>
                          </td>
